...
description: Capture a single Lenny's Newsletter episode (podcast interview or written post) into a faithful, structured bilingual (English + 中文) raw-material document — full transcript read end-to-end, themes captured with their argument structure (why/thresholds/qualifications/anti-patterns), concrete examples, verbatim quotes with timestamps, and transcription artifacts and contestable points flagged. This skill produces RAW MATERIAL ONLY: it never ranks themes by importance, never writes recommendations, takeaways, or action items, and never tells the reader what they should do — those judgments belong to the reader downstream and capturing them here would defeat the skill's purpose. Use this whenever the user wants to digest, summarize, capture, write up, or "run C1 on" a Lenny episode — including when they paste a Lenny URL, name an episode or guest, or say things like "process the latest Lenny one," "do the capture for this episode," or "turn this Lenny transcript into the raw layer." Trigger even if they don't say the words "capture" or "skill."
---
